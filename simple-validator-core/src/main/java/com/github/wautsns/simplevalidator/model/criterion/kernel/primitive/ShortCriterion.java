...
public interface ShortCriterion extends PrimitiveCriterion<Short> {
    ...
    @Override
    default ValidationFailure testWrappedValue(Short wrappedValue) {
        return test(wrappedValue);
    }

    @Override
    default ShortCriterion enhanceResultProcessing(UnaryOperator<ValidationFailure> adjuster) {
        return value -> adjuster.apply(test(value));
    }

    @Override
    default TCriterion<?> wrapFieldIntoType(Field field) {
        return source -> test(ReflectionUtils.getFieldShort(source, field));
    }

    @Override
    default TCriterion<short[]> wrapComponentIntoArray() {
        return array -> {
