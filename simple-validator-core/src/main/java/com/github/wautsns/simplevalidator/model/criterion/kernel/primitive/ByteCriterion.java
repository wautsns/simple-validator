...
public interface ByteCriterion extends PrimitiveCriterion<Byte> {
    ...
    @Override
    default ValidationFailure testWrappedValue(Byte wrappedValue) {
        return test(wrappedValue);
    }

    @Override
    default ByteCriterion enhanceResultProcessing(UnaryOperator<ValidationFailure> adjuster) {
        return value -> adjuster.apply(test(value));
    }

    @Override
    default TCriterion<?> wrapFieldIntoType(Field field) {
        return source -> test(ReflectionUtils.getFieldByte(source, field));
    }

    @Override
    default TCriterion<byte[]> wrapComponentIntoArray() {
        return array -> {
