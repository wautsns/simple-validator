 ...
 * @since Mar 11, 2020
 */
public abstract class CriterionForFloat extends CriterionForPrimitive<Float> {

    /** Truth. */
    public static final CriterionForFloat TRUTH = new CriterionForFloat() {
        @Override
     ...
     */
    public final ValidationFailure test(float value) {
    ...
    @Override
    public ValidationFailure testWrappedPrimitiveValue(Float value) {
        return test(value);
    }

    /**
     * Test {@code float} value without enhancing failure.
     *
     * @param value {@code float} value
     * @return validation failure without enhancing failure, or {@code null} if the validation is passed.
     */
    protected abstract ValidationFailure testWithoutEnhancingFailure(float value);
