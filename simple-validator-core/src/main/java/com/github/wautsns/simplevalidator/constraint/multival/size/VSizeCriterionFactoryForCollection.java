...
/**
 * VSize criterion factory for {@code Collection} value.
 *
 * @author wautsns
 * @since Mar 11, 2020
 */
@NoArgsConstructor(access = AccessLevel.PRIVATE)
public class VSizeCriterionFactoryForCollection extends CriterionFactoryForNonPrimitive<VSize, Collection<?>> {

    /** {@code VSizeCriterionFactoryForCollection} instance. */
    public static final VSizeCriterionFactoryForCollection INSTANCE = new VSizeCriterionFactoryForCollection();

    @Override
    public boolean applyTo(Type type, VSize constraint) {
        return TypeUtils.isAssignableTo(type, Collection.class);
    }

    @Override
    public void process(ConstrainedNode node, VSize constraint, CriteriaForNonPrimitive<Collection<?>> wip) {
     ...
     * @return criterion
     */
    protected CriterionForNonPrimitive<Collection<?>> produce(VSize constraint) {
        int min = constraint.min();
        int max = constraint.max();
        return new CriterionForNonPrimitive<Collection<?>>() {
            @Override
            protected ValidationFailure testWithoutEnhancingFailure(Collection<?> value) {
                int size = value.size();
