...
/**
 * Criteria for {@code byte} value.
 *
 * @author wautsns
 * @since Mar 11, 2020
 */
public class CriteriaForByte extends CriterionForByte implements Criteria<CriterionForByte> {

    /** Whether the criteria support {@code null} value. */
    private boolean nullable;
    /** Original criteria. */
    private final List<CriterionForByte> originalCriteria = new LinkedList<>();
    ...
    @Override
    public List<CriterionForByte> getOriginalCriteria() {
        return originalCriteria;
    }

    @Override
    protected ValidationFailure testWithoutEnhancingFailure(byte value) {
        for (CriterionForByte criterion : originalCriteria) {
