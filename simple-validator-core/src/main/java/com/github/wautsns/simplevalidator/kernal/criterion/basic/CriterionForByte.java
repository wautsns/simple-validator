...
/**
 * Criterion for {@code byte} value.
 *
 * @author wautsns
 * @since Mar 11, 2020
 */
public abstract class CriterionForByte extends CriterionForPrimitive<Byte> {

    /** Truth. */
    public static final CriterionForByte TRUTH = new CriterionForByte() {
        @Override
     ...
     */
    public final ValidationFailure test(byte value) {
    ...
    @Override
    public ValidationFailure testWrappedPrimitiveValue(Byte value) {
        return test(value);
    }

    /**
     * Test {@code byte} value without enhancing failure.
     *
     * @param value {@code byte} value
     * @return validation failure without enhancing failure, or {@code null} if the validation is passed.
     */
    protected abstract ValidationFailure testWithoutEnhancingFailure(byte value);
