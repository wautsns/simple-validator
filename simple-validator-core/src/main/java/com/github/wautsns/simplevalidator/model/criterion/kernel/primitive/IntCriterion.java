...
public interface IntCriterion extends PrimitiveCriterion<Integer> {
    ...
    @Override
    default IntCriterion enhanceResultProcessing(UnaryOperator<ValidationFailure> adjuster) {
        return value -> adjuster.apply(test(value));
    }

    @Override
    default TCriterion<?> wrapFieldIntoType(Field field) {
        return source -> test(ReflectionUtils.getFieldInt(source, field));
    }

    @Override
    default TCriterion<int[]> wrapComponentIntoArray() {
        return array -> {
