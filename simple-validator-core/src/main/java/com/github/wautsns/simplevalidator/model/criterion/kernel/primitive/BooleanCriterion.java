...
public interface BooleanCriterion extends PrimitiveCriterion<Boolean> {
    ...
    @Override
    default BooleanCriterion enhanceResultProcessing(UnaryOperator<ValidationFailure> adjuster) {
        return value -> adjuster.apply(test(value));
    }

    @Override
    default TCriterion<Boolean> wrapFieldIntoType(Field field) {
        return source -> test(ReflectionUtils.getFieldBoolean(source, field));
    }

    @Override
    default TCriterion<boolean[]> wrapComponentIntoArray() {
        return array -> {
