...
public interface CharCriterion extends PrimitiveCriterion<Character> {
    ...
    @Override
    default CharCriterion enhanceResultProcessing(UnaryOperator<ValidationFailure> adjuster) {
        return value -> adjuster.apply(test(value));
    }

    @Override
    default TCriterion<?> wrapFieldIntoType(Field field) {
        return source -> test(ReflectionUtils.getFieldChar(source, field));
    }

    @Override
    default TCriterion<char[]> wrapComponentIntoArray() {
        return array -> {
