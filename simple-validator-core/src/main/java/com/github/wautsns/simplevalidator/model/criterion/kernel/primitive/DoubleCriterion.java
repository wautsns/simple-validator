...
public interface DoubleCriterion extends PrimitiveCriterion<Double> {
    ...
    @Override
    default ValidationFailure testWrappedValue(Double wrappedValue) {
        return test(wrappedValue);
    }

    @Override
    default DoubleCriterion enhanceResultProcessing(UnaryOperator<ValidationFailure> adjuster) {
        return value -> adjuster.apply(test(value));
    }

    @Override
    default TCriterion<?> wrapFieldIntoType(Field field) {
        return source -> test(ReflectionUtils.getFieldDouble(source, field));
    }

    @Override
    default TCriterion<double[]> wrapComponentIntoArray() {
        return array -> {
